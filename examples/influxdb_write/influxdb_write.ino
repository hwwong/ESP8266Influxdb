...
void loop() {
  ...
  String data =
      "analog_read,method=HTTP_API,pin=A0 value=" + String(analogRead(A0));
  dbresponse = influxdb.write(data);
  Serial.println(dbresponse == DB_SUCCESS ? "Writing Success"
                                          : "Writing failed");

  // Writing data using FIELD object
  FIELD dataObj(
      "analog_read"); // Create field object with measurment name=analog_read
